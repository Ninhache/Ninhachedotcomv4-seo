...
export default function Home({ profile, locale }: Props) {
    const t = useTranslations('home');
    const t_about = useTranslations('about');

    const trans = profile?.translations.find(t => t.locale === locale);

    const greeting = trans?.greeting || t('greeting');
    const name = profile?.name || t('name');
    const profession = trans?.profession || t('profession');
    const description = trans?.description || t('description');
    ...
    return (
        <>
            <section
                id={t('anchor')}
                className={`section ${styles.home_section}`}
            >
                {memoizedParticlesComponent}
                <div className={`content ${styles.content}`}>
                    <AnimatedComponent delay={100}>
                        <span
                            className={`${styles.hi} ${ralewaySemiBold.className}`}
                        >
                            {greeting}
                        </span>
                    </AnimatedComponent>
                    ...
                    <AnimatedComponent delay={400}>
                        <p
                            className={`${styles.bio} ${calibreRegular.className}`}
                        >
                            {description}
                        </p>
                    </AnimatedComponent>
