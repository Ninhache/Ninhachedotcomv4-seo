      ...
      <div className={`${styles.menu}`}>
        {data.map((category, index) => (
          <SkillCategoryView
            ...
            onClick={() => setSelectedCategory(category.translations[locale].name)}
            selectioned={selectedCategory === category.translations[locale].name}
          />
        ))}
      </div>
      <div className={`${styles.box_content}`}>
        {data.map((category, index) => (
          <SkillsList
            ...
            isVisible={category.translations[locale].name === selectedCategory}
          />
        ))}
      </div>
    </>
...
const SkillsList: React.FC<SkillsListProps> = ({ skills, isVisible }) => {
  return (
    <div className={`${styles.skills_list}`} style={{ display: isVisible ? 'grid' : 'none' }}>
