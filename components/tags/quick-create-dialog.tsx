...
export function TagQuickCreateDialog({
    ...
    const defaultValues = React.useMemo<FormInput>(() => {
        const translations = Object.fromEntries(
            (locales?.length ? locales : ['fr', 'en']).map(loc => [
                loc,
                { name: initialName || '' },
            ])
        );
    ...
    const previewName =
        form.watch(`translations.${primaryLocale}.name`) || 'Aperçu';
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-lg">
                ...
                <form
                    onSubmit={e => {
                        // Stop the synthetic event from bubbling up the React tree
                        // to any parent <form> (e.g. ProjectForm rendered in an outer Dialog).
                        e.preventDefault();
                        e.stopPropagation();
                        onSubmit(e);
                    }}
